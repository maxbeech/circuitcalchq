...
import { COMMON_BOXES, calcBoxFill } from "@/lib/box-fill";
...
export default function BoxFillCalculator() {
  ...
  const r = calcBoxFill({ conductors, devices, clamps, supports: 0, grounds, groundSize: "14", boxVolume: box.volume });
    ...
    <div className="grid gap-5 md:grid-cols-2">
      <div className="space-y-4">
          ...
          <div className="space-y-2">
            {conductors.map((w, i) => (
              <div key={i} className="flex items-center gap-2">
                <NumberInput value={w.count} min={0} onChange={(e) => update(i, { count: Math.max(0, +e.target.value) })} className="w-20 rounded-lg border border-slate-300 bg-white px-3 py-2" />
                <span className="text-slate-400">×</span>
                <Select value={w.size} onChange={(e) => update(i, { size: e.target.value })}>
                  {SIZES.map((s) => <option key={s} value={s}>{sizeLabel(s)}</option>)}
                </Select>
                <button type="button" onClick={() => setConductors((w) => w.filter((_, j) => j !== i))} className="rounded-lg px-2 py-1 text-slate-400 hover:bg-slate-100 hover:text-rose-600" aria-label="Remove">✕</button>
              </div>
            ))}
          </div>
        ...
        <Field label="Device yokes (switches / receptacles)">
          <NumberInput value={devices} min={0} onChange={(e) => setDevices(Math.max(0, +e.target.value))} />
        </Field>
