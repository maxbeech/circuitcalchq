      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50 text-left text-slate-600">
            ...
            <th className="px-3 py-2 text-right font-semibold" rowSpan={2}>R Al Ω/kft</th>
          </tr>
          <tr className="border-b border-slate-200 bg-slate-50 text-center text-xs text-slate-500">
        ...
        <tbody>
          {CONDUCTORS.map((c) => (
            <tr key={c.size} className="border-b border-slate-100 last:border-0">
              ...
              <td className="px-3 py-1.5 text-right text-slate-500">{c.ohm.al}</td>
            </tr>
          ))}
        </tbody>
      </table>
