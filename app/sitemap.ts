...
import { CONDUCTORS } from "@/lib/nec-conductors";
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const urls: MetadataRoute.Sitemap = [
    ...
    { url: `${SITE.url}/calculators`, lastModified: now, priority: 0.9 },
    { url: `${SITE.url}/wire`, lastModified: now, priority: 0.8 },
  ...
  for (const c of CALCS) urls.push({ url: `${SITE.url}/calculators/${c.slug}`, lastModified: now, priority: 0.9 });
  for (const a of AMP_TARGETS) urls.push({ url: `${SITE.url}/amps/${a}`, lastModified: now, priority: 0.7 });
  for (const c of CONDUCTORS) urls.push({ url: `${SITE.url}/wire/${gaugeSlug(c.size)}`, lastModified: now, priority: 0.6 });
